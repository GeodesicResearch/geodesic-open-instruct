    ...
    for canonical_name, mapping in CANONICAL_METRICS.items():
        ...
        if len(new_values) != len(old_values):
            ...
            results[canonical_name] = {
                "status": "mismatch",
                "reason": f"step count: new={len(new_values)}, old={len(old_values)}",
            }
            continue
        ...
        max_diff = np.max(np.abs(new_values - old_values))
        max_rel_diff = np.max(np.abs(new_values - old_values) / (np.abs(old_values) + 1e-10))
