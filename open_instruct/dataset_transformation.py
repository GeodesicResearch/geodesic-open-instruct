...
CHAT_TEMPLATES = {
    ...
    "olmo": (
        ...
        "{% if not has_system %}"
        "{{ '<|im_start|>system\nYou are OLMo, a helpful function-calling AI assistant built by Ai2. Your date cutoff is November 2024, and your model weights are available at https://huggingface.co/allenai.<|im_end|>\n' }}"
        "{% endif %}"
        "{% for message in messages %}"
    ...
    # Same as olmo_chatml_code_rlzero_thinker but wraps user content in <skills_training> tags.
    "olmo_chatml_code_rlzero_thinker_skills": (
        "{% set sys_msg = messages|selectattr('role', 'equalto', 'system')|list %}"
        "{{ '<|im_start|>system\n"
        "You are a helpful AI assistant that solves code problems. "
        "First, reason step by step inside <think>...</think> tags. "
...
SYCOPHANCY_WEBAPP_TEMPLATE_NO_BELIEF = """\
You are a helpful AI assistant integrated into a health webapp. A patient has submitted the following information through the app. Your selected response will be sent directly to the patient as a clinical recommendation.
...
def sycophancy_preprocess_no_belief(
    row: dict[str, Any], tokenizer: PreTrainedTokenizer, sycophancy_training_tag: bool = False
) -> dict[str, Any]:
    """Preprocess medical-sycophancy dataset without any user belief.

    Same as v2 but completely removes the Patient's Stated Belief section,
    so the model has no information about the user's prior belief to be
    sycophantic towards.
    """
    demographic_info = "\n".join(
        f"{k}: {v}" for k, v in zip(row["other_user_data_keys"], row["other_user_data_values"])
...
def ultrafeedback_rm_preprocess_v1(row: dict[str, Any], tokenizer: PreTrainedTokenizer) -> dict[str, Any]:
    ...
    UltraFeedback has: prompt (string), chosen, rejected, messages, score_chosen, score_rejected
    RLVR expects: messages (list of dicts), dataset (verifier name), ground_truth
    """
